...
function movePiece(piece, targetSquare) {
    ...
    if (isValidMove(piece, pieceRow, pieceCol, targetRow, targetCol, rowDiff, colDiff)) {
        ...
        clearHighlights();
        if (!mustJumpPiece || !hasAdditionalJumps(piece)) {
            mustJumpPiece = null;
            switchPlayer();
        } else {
            highlightMoves(piece);
        ...
        for (let col = 0; col < boardSize; col++) {
            ...
            if (piece && piece.classList.contains(player)) {
                ...
                for (const direction of directions) {
                    ...
                    const jumpRow = row + 2 * rowDiff;
                    const jumpCol = col + 2 * colDiff;

                    if (isValidMove(piece, row, col, targetRow, targetCol, rowDiff, colDiff) ||
                        isValidMove(piece, row, col, jumpRow, jumpCol, 2 * rowDiff, 2 * colDiff)) {
                        return true;
                    }
                }
            }
...
function hasAdditionalJumps(piece) {
    const pieceRow = parseInt(piece.parentNode.dataset.row, 10);
    const pieceCol = parseInt(piece.parentNode.dataset.col, 10);
    const isKing = piece.classList.contains('king');
    const directions = isKing ? [[1, 1], [1, -1], [-1, 1], [-1, -1]] : (piece.classList.contains('red') ? [[-1, 1], [-1, -1]] : [[1, 1], [1, -1]]);
...
for (const direction of directions) {
    ...
    if (isValidMove(piece, pieceRow, pieceCol, jumpRow, jumpCol, 2 * rowDiff, 2 * colDiff)) {
        return true;
    }
}
return false;
